  ...
  return (
    <nav className="sticky top-0 z-[100] bg-white border-b border-gray-100 w-full h-[70px] md:h-[80px] flex items-center justify-between px-4 sm:px-10 shadow-sm">
      ...
      <div className="hidden md:flex items-center space-x-2 font-semibold">
        <Link to="/" className={location.pathname === '/' ? activeStyle : idleStyle}><HiOutlineHome size={22}/> <span>Asosiy</span></Link>
        {user && (
          <>
            <Link to="/all-products" className={location.pathname === '/all-products' ? activeStyle : idleStyle}><AiOutlineShoppingCart size={22}/> <span>Mahsulotlar</span></Link>
            <Link to="/add-products" className={location.pathname === '/add-products' ? activeStyle : idleStyle}><AiOutlinePlusSquare size={22}/> <span>Qo'shish</span></Link>
          </>
        )}
          ...
          user ? (
            <div className="relative" ref={profileRef}>
              <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="focus:outline-none">
                <img src={user.photoURL || 'https://via.placeholder.com/40'} className="w-10 h-10 rounded-full border-2 border-transparent hover:border-purple-600 transition-all object-cover" />
              </button>
              {isProfileOpen && (
                <div className="absolute right-0 mt-3 w-64 bg-white border rounded-2xl shadow-2xl z-[120]">
                  ...
                  <div className="py-2">
                    <Link to="/my-profile" className="flex items-center space-x-3 px-4 py-3 hover:bg-purple-50 text-gray-700 font-semibold"><HiOutlineUserCircle size={20}/> <span>Profilim</span></Link>
                    <button onClick={handleLogout} className="w-full flex items-center space-x-3 px-4 py-3 hover:bg-red-50 text-red-600 font-semibold border-t"><HiLogout size={20}/> <span>Chiqish</span></button>
                  </div>
                </div>
              )}
            </div>
          ) : (
            <button onClick={handleLogin} className="bg-purple-600 text-white px-6 py-2 rounded-xl font-bold shadow-md hover:bg-purple-700 transition-all">Kirish</button>
          )
        )}
      </div>
      ...
      <div className="md:hidden flex items-center">
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 z-[200] relative">
      ...
      <div className={`fixed inset-0 z-[150] transition-all duration-300 ${isMobileMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`absolute right-0 top-0 h-[100dvh] w-[300px] bg-white shadow-2xl transition-transform duration-300 flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {user && (
              <div className="mb-6 p-4 bg-gradient-to-br from-purple-50 to-white rounded-2xl border border-purple-100 flex items-center space-x-4">
                <img src={user.photoURL} className="w-12 h-12 rounded-full border-2 border-white shadow-sm" alt="p" />
          ...
          <div className="p-4 border-t bg-gray-50 flex-shrink-0">
            {user ? (
              <button onClick={handleLogout} className="w-full flex items-center justify-center space-x-3 bg-red-50 text-red-600 p-4 rounded-xl font-bold active:scale-95 transition-all">
                <HiLogout size={22}/> <span>Tizimdan Chiqish</span>
              </button>
            ) : (
              <button onClick={handleLogin} className="w-full bg-purple-600 text-white p-4 rounded-xl font-bold shadow-lg">
